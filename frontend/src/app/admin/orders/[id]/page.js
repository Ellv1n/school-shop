...
import { formatPrice, getImageUrl, ORDER_STATUS_LABELS, getErrorMessage } from '@/lib/utils';
...
export default function AdminOrderDetailPage() {
  ...
  const { data: order, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get('/orders/admin/all', { params: { limit: 200 } });
    ...
    <div className="max-w-3xl space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm flex items-center gap-1 mt-0.5">
            <Clock className="w-3.5 h-3.5" /> {safeDate(createdAt)}
          </p>
        </div>
      </div>
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div>
              ...
              {(order.user?.email || order.user_email) && (
                <p className="text-xs text-slate-500 mt-0.5">{order.user?.email || order.user_email}</p>
              )}
            </div>
          </div>
          <div className="flex items-start gap-2.5">
      ...
      <div className="card p-6">
              ...
              return (
                <li key={item.id || idx} className="flex gap-4 items-center p-3 rounded-xl hover:bg-slate-50 transition-colors">
                  <div className="w-14 h-14 rounded-xl overflow-hidden bg-slate-100 flex-shrink-0">
                    {imgUrl ? (
                      <Image src={imgUrl} alt={name} width={56} height={56}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-slate-500 mt-0.5">{qty} ədəd × {formatPrice(price)}</p>
                  </div>
                  <p className="font-bold text-slate-800 flex-shrink-0">{formatPrice(parseFloat(price) * qty)}</p>
                </li>
              );
            })}
          </ul>
        ...
        <div className="border-t border-slate-100 mt-4 pt-4 flex justify-between items-center">
          <span className="font-medium text-slate-600">Ümumi məbləğ:</span>
          <span className="font-display font-bold text-xl text-primary-700">{formatPrice(totalPrice)}</span>
        </div>
      </div>

      <Link href="/admin/orders" className="btn btn-secondary inline-flex">
        ← Sifarişlər siyahısı
      </Link>
    </div>
  );
