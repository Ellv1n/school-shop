...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <StatCard icon={TrendingUp} label="Ümumi gəlir"   value={formatPrice(stats.totalRevenue)} color="bg-green-100 text-green-600" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 card p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link href="/admin/orders" className="text-sm text-primary-600 hover:underline font-medium">Hamısına bax →</Link>
          </div>
                ...
                <Link key={order.id} href={`/admin/orders/${order.id}`}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm text-slate-800 truncate">
                      {order.user?.name} · <span className="font-mono text-xs">#{order.id.slice(0, 8).toUpperCase()}</span>
                    </p>
                    <p className="text-xs text-slate-400">{formatDate(order.createdAt)}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="font-semibold text-sm text-slate-800">{formatPrice(order.totalPrice)}</p>
              ...
              {ordersByStatus.map(({ status, _count }) => {
                ...
                return (
                  <div key={status} className="flex items-center justify-between">
                    ...
                    <span className="font-bold text-slate-700">{_count.status}</span>
                  </div>
                );
              })}
            </div>
              ...
              <div className="space-y-2">
                {lowStock.slice(0, 5).map(product => (
                  <Link key={product.id} href={`/admin/products/${product.id}/edit`}
                    ...
                    <span className="text-slate-700 truncate">{product.nameAz}</span>
