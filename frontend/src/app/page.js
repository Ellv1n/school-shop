    ...
    <>
      ...
      <main>
        ...
        <section className="page-container py-14">
          ...
          <Suspense fallback={<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">{Array.from({length:6}).map((_,i)=><div key={i} className="skeleton h-32 rounded-2xl"/>)}</div>}>
            <CategoryGrid />
          </Suspense>
        </section>
        ...
        <section className="bg-white border-y border-slate-100 py-14">
          <div className="page-container">
            <div className="flex items-end justify-between mb-8">
              ...
              <Link href="/products?featured=true" className="btn btn-secondary btn-sm hidden sm:flex">
                Hamısını gör <ArrowRight className="w-4 h-4" />
              </Link>
            </div>
            <Suspense fallback={<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">{Array.from({length:8}).map((_,i)=><div key={i} className="skeleton h-72 rounded-2xl"/>)}</div>}>
              <FeaturedProducts />
            </Suspense>
          </div>
        </section>
